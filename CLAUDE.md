...
- `go tool task coverage` — enforces **100% hand-written statement coverage** for product code (excludes `internal/client/internal/gql/generated.go`, `cmd/linctl/main.go`, and repo maintenance scripts under `scripts/`). Not part of `task ci`; run it separately and keep new product code fully covered.
