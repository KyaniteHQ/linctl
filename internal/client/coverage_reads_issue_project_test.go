...
func Test_ClientReadIssueAndProjectScenarios_return_compact_lists_details_and_members(t *testing.T) {
	...
	require.NoError(t, err)
	projectDetail, err := GetProjectDetail(context.Background(), graphqlClient, "project-id")
	require.NoError(t, err)
	...
	require.Equal(t, "Existing project content", projectDetail.Content)
	require.Equal(t, project.ID, projectDetail.Summary.ID)
