...
func Test_CommandFlows_print_json_for_read_and_comment_commands(t *testing.T) {
	tests := []struct {
		...
		{args: []string{"--json", "--fields", "id", "release", "search", "mobile", "--limit", "1"}, keys: []string{"releases"}},
		...
		{args: []string{"--json", "--fields", "identifier,title", "issue", "list", "--limit", "1"}, keys: []string{"issues"}},
		{args: []string{"--json", "issue", "search", "needle", "--limit", "1"}, keys: []string{"issues", "has_next_page"}},
		{args: []string{"--json", "--fields", "identifier,title", "issue", "search", "needle", "--limit", "1"}, keys: []string{"issues"}},
		{args: []string{"--json", "--fields", "name", "project", "get", "project-id"}, keys: []string{"name"}},
		...
		{args: []string{"--json", "project-milestone", "delete", "project-milestone-id"}, keys: []string{"id", "status"}},
...
func Test_CommandFlows_print_only_id_when_id_only_flag_is_set(t *testing.T) {
	tests := []struct {
		...
		{name: "issue unrelate", args: []string{"--id-only", "issue", "unrelate", "issue-relation-id"}, output: "issue-relation-id\n"},
		{
			...
			args:   []string{"--id-only", "project-milestone", "delete", "project-milestone-id"},
			output: "project-milestone-id\n",
		},
...
func Test_CommandFlows_suppress_success_output_when_quiet_flag_is_set(t *testing.T) {
	tests := [][]string{
		...
		{"--quiet", "project-milestone", "delete", "project-milestone-id"},
