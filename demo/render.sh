...
#   - a Linear OAuth access token for a THROWAWAY demo target in LINCTL_OAUTH_ACCESS_TOKEN
...
# Usage: LINCTL_OAUTH_ACCESS_TOKEN=<oauth-access-token> ./render.sh
set -euo pipefail
cd "$(dirname "$0")"

: "${LINCTL_OAUTH_ACCESS_TOKEN:?set LINCTL_OAUTH_ACCESS_TOKEN to a Linear OAuth access token for the demo target}"
