...
`go tool task ci` is the local review gate. It verifies module checksums and tidy state,
formatting, generated artifacts, domain language, vet, tests, build, CLI smoke output,
golangci-lint, ShellCheck, actionlint, and govulncheck without modifying source files.
It also verifies `docs/linear-api-coverage.md` against the upstream Linear SDK checkout.
Clone or refresh that checkout at `/tmp/linctl-upstream-linear`, or set
`LINCTL_LINEAR_SDK_UPSTREAM`, before running the full gate:

```bash
git clone https://github.com/linear/linear.git /tmp/linctl-upstream-linear
```
