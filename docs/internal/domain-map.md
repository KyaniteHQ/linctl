...
| `issue import` | Reads a CSV or JSON file (format from extension). Each row's `state` is an exact workflow-state name, matching `issue create --state`. Priority still normalizes. Rejects any row whose `team` key ≠ the pinned `team_key`. Creates each issue via guarded `Mutation.issueCreate` (`CreateIssue`). `--dry-run` loads only local config, renders the rows locally, and performs no auth, network, or mutation setup | Team-scoped per row; each create re-runs the write guard; `--dry-run` writes nothing |
| `issue bulk-export` | `Query.issues` filtered to the resolved team (`ListIssuesByTeam` with an empty filter set) written to a CSV or JSON file (format from extension), capped by `--limit` (default 250) | Read-only, writes only the local file |
...
`issue-relation list` and `issue-relation get` (reads) plus `issue relate ISSUE RELATED --type` and `issue unrelate RELATION_ID` (writes) are implemented in the current CLI. `issue relate` resolves both endpoints so a relation lands only when both issues belong to the resolved organization and team. A pinned project stays allowed. `--allowed-project` names every extra project those issues may occupy. Relating across projects without that flag is an invalid write. A project outside the allowed set is a Target Mismatch. A missing organization id on an issue is a hard stop. A cross-organization pair returns `CROSS_ORGANIZATION_RELATION` before mutation. The write does not move either issue. A `blocks` relation is rejected when the related issue already blocks the source issue. Success reads both issues and the relation back and prints that full result. `issue unrelate` is the one approved relation delete; it resolves the relation and confirms both linked issues against the same organization, team, and allowed-project boundary before removing it. `issue-relation update` stays deferred until its endpoint guard model is explicit.
...
| `comment resolve` | `Mutation.commentResolve` | Resolve the comment, then compare the pinned team through its parent issue before resolving the thread; non-issue comments are refused |
| `comment unresolve` | `Mutation.commentUnresolve` | Resolve the comment, then compare the pinned team through its parent issue before reopening the thread; non-issue comments are refused |

`comment list`, `comment get`, `comment bot-actor`, `comment children`, and `comment created-issues` (reads) plus `comment update COMMENT_ID --body`, `comment delete COMMENT_ID`, `comment resolve COMMENT_ID`, and `comment unresolve COMMENT_ID` (writes) are implemented in the current CLI. All four writes resolve the comment, then compare the pinned team through the comment's parent issue (`guardCommentTarget`): a comment not attached to an issue is refused because the issue guard cannot prove its target. Resolve and unresolve pass the supplied comment id directly to Linear, which normalizes child comments to their root thread. `comment delete` is the one approved delete. Comment child reads omit comment body content by default. Document content and external thread reads remain out of the default surface because they expose content/thread payloads. Issue-scoped comment creation and replies remain under the guarded `issue comment` and `issue reply` commands.
...
| `project add-team` | `Mutation.projectUpdate` with `ProjectUpdateInput.teamIds`; merges the destination onto the complete current membership (Linear replaces the full set), refuses when membership cannot be fully enumerated, and fails closed if the pin's team is dropped | Resource-scoped on the project; destination compared organization-only |
