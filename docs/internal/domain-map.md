...
`issue-relation list` and `issue-relation get` (reads) plus `issue relate ISSUE RELATED --type` and `issue unrelate RELATION_ID` (writes) are implemented in the current CLI. `issue relate` resolves both endpoints so a relation lands only when both issues belong to the resolved organization and team. `--allowed-project` names every project those issues may occupy; a project outside that list is a Target Mismatch. A cross-organization pair returns one specific error before mutation. The write does not move either issue. A `blocks` relation is rejected when the related issue already blocks the source issue. Success reads both issues and the relation back. `issue unrelate` is the one approved relation delete; it resolves the relation and confirms both linked issues before removing it. `issue-relation update` stays deferred until its endpoint guard model is explicit.
